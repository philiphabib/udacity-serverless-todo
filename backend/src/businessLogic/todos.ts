...
export async function putAttachment(userId: string, todoId: string, s3Bucket: string){
...
export async function createTodo( newToDo: { name: string; dueDate: string; todoId: string; userId: string; } ){
  await docClient.put({
    TableName: todosTable,
    Item: newToDo,
  }).promise()
